...
function NyzScriptsLibrary:CreateWindow(config)
	...
	-- FIX 2: ClipsDescendants = true supaya konten ga meluber keluar frame
	...
	local LightningCanvas = Instance.new("Frame", MainClip)
	...
	local function generateLightning(parent, x1, y1, x2, y2, depth)
		...
		if math.random() > 0.55 then
			generateLightning(parent, mx, my, mx + math.random(-60, 60), my + math.random(-60, 60), depth - 2)
		end
	...
	for i = 1, FRAME_COUNT do
		...
		for _ = 1, math.random(2, 4) do
			...
			generateLightning(holder, x1, y1, math.random(W*0.2, W*0.8), math.random(H*0.2, H*0.8), 3)
		end
	...
	function Window:AddTab(tabConfig)
		...
		-- FIX 1: ClipsDescendants = true supaya konten ga keluar saat scroll
		...
		function Tab:AddButton(btnConfig)
			...
			clickHint.Size = UDim2.new(0, 28, 0, 20)
			clickHint.AnchorPoint = Vector2.new(1, 0.5)
			clickHint.Position = UDim2.new(1, -10, 0.5, 0)
			clickHint.BackgroundTransparency = 1
			clickHint.Text = "click"
			clickHint.TextColor3 = Color3.fromRGB(180, 130, 0)
